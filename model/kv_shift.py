...
class TokenShift(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, prev_weight: torch.Tensor, curr_weight: torch.Tensor):
        ...
        if prev_weight.size() != (batch_size, seq_len, num_heads) or curr_weight.size() != (batch_size, seq_len, num_heads):
        ...
        grid = lambda meta: (batch_size, triton.cdiv(seq_len, meta["BLOCK_T"]), num_heads)
    ...
    def forward(self, x: torch.Tensor, shift_state: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if seq_len > 1:
            ...
            if shift_state is None:
                ...
                result_per_head = alpha.unsqueeze(-1) * shift_state_per_head + (1.0 - alpha).unsqueeze(-1) * out_per_head
