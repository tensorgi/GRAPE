...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        omega_init = getattr(config, "querykeygated_omega_init", None)
        u_init_std = getattr(config, "keygated_u_init_std", None)
        v_init_std = getattr(config, "querygated_v_init_std", None)
        u_l2_norm = bool(getattr(config, "keygated_u_l2_norm", True))
        u_l2_eps = float(getattr(config, "keygated_u_l2_eps", 1e-6))
        v_l2_norm = bool(getattr(config, "querygated_v_l2_norm", True))
        v_l2_eps = float(getattr(config, "querygated_v_l2_eps", 1e-6))
        self.querykeygated_bias = QueryKeyGatedAdditiveBias(
            self.n_head,
            self.head_dim,
            omega_init=omega_init,
            u_init_std=u_init_std,
            v_init_std=v_init_std,
            u_l2_norm=u_l2_norm,
            u_l2_eps=u_l2_eps,
            v_l2_norm=v_l2_norm,
            v_l2_eps=v_l2_eps,
    ...
    def forward(self, x):
        ...
        # Query+key-gated additive bias, broadcastable to (B, H, T, T)
        bias = self.querykeygated_bias(q, k).float()
...
class GPT(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        init_llama_mha_weights(
            self, config, exclude_suffixes=("querykeygated_bias.u", "querykeygated_bias.v")
        )
