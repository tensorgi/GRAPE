...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        omega_init = getattr(config, "querygated_omega_init", None)
        ...
        self.querygated_bias = QueryGatedAdditiveBias(
            self.n_head,
            self.head_dim,
    ...
    def forward(self, x):
        ...
        # Query-gated additive bias, broadcastable to (B, H, T, T)
        bias = self.querygated_bias(q).float()
...
class GPT(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        init_llama_mha_weights(self, config, exclude_suffixes=("querygated_bias.v",))
